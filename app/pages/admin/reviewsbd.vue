...
const imageTimestamps = ref<Record<number, number>>({})
          ...
          <tbody>
            ...
            <tr v-for="review in reviews" :key="review.id">
              ...
              <td>
                <img
                  v-if="review.photo"
                  :src="(review.photo.startsWith('http') ? review.photo : `${apiUrl}/${String(review.photo).replace(/^\/+/, '')}`) + `?t=${imageTimestamps[review.id] || 0}`"
                  alt="review photo"
                  class="thumb"
                />
                <span v-else>—</span>
